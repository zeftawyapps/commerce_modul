...
class CommerceHttpClient {
  ...
  CommerceHttpClient({this.baseUrl   , userToken = false}) {
  ...
  Future<CommerceResult<RemoteBaseModel, Map<String , dynamic> >> sendRequestResult<T>({
    required HttpMethod method,
    required String url,
    Map<String, String>? headers,
    Map<String, dynamic>? queryParameters,
    ...
    try {
      ...
      try {
        ...
        Map<String ,dynamic> data = { "status" : response.data!["status"]  , "data": response.data??"" } ;
    ...
    on DioError catch (e) {
      ...
      return CommerceResult. error(RemoteBaseModel(message: error["massage"] , status: "error" , data:  "null"));
    }
